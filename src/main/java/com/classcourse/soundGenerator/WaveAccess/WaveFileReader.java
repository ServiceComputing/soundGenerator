package com.classcourse.soundGenerator.WaveAccess;
...
public class WaveFileReader {
    ...
    private String junkflag = null;
    private String fmtsubchunk = null;
    private long junkchunksize = 0;
    ...
    private void initReader(String filename) {
        ...
        try {
            ...
            // for JUNK SubChunk
            this.junkflag = readString(WaveConstants.LENJUNKFLAG);
            if (!junkflag.endsWith("JUNK"))
                throw new IllegalArgumentException("JUNK miss, " + filename + " may not be created by Apple.");
            this.junkchunksize = readLong();
            readString((int) this.junkchunksize);
            ...
            // for FLLR SubChunk
            this.datasubchunk = readString(WaveConstants.LENFLLRFLAG);
            if (!datasubchunk.endsWith("FLLR"))
                throw new IllegalArgumentException("FLLR miss, " + filename + " may not be created by Apple.");
            this.subchunk2size = readLong();
            readString((int) this.subchunk2size);
